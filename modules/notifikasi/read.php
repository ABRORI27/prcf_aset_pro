...
// ✅ Perbaikan kolom kode_penomoran dan relasi aset_id
$sql = "SELECT n.*, a.nama_barang, a.kode_penomoran AS kode_barang
        FROM notifikasi n
...
if (!$result) {
    die("Query gagal: " . mysqli_error($conn) . "<br>SQL: " . $sql);
}
?>
      ...
      <tbody>
        ...
        if (mysqli_num_rows($result) > 0) {
          while ($row = mysqli_fetch_assoc($result)) {
            ...
            // Warna status berdasar waktu
            $color = ($tgl < $today) ? 'red' : (($diff <= 7) ? 'yellow' : 'green');

            // Pastikan kolom tampil walau aset dihapus
